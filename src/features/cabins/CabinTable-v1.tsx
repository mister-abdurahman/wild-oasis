...
function CabinTable() {
  ...
  switch (filterData) {
    case "all":
      ...
      break;
    case "with-discount":
      filteredData = cabin?.filter((cab) => cab.discount > 0);
      break;
    case "no-discount":
      filteredData = cabin?.filter((cab) => cab.discount < 1);
      break;
    default:
      filteredData = cabin;
      break;
  ...
  // SORT:
  const sortData = searchParams.get("sortBy") || "name-asc";
  ...
  // switch (sortData) {
  //   case "name-asc":
  //     filteredData = cabin?.sort((a, b) => a.name - b.name);
  //     break;
  //   case "name-desc":
  //     filteredData = cabin?.sort((a, b) => b.name - a.name);
  //     break;
  //   case "regularPrice-asc":
  //     filteredData = cabin?.sort((a, b) => a.price - b.price);
  //     break;
  //   case "regularPrice-desc":
  //     filteredData = cabin?.sort((a, b) => b.price - a.price);
  //     break;
  //   case "maxCapacity-asc":
  //     filteredData = cabin?.sort((a, b) => a.capacity - b.capacity);
  //     break;
  //   case "maxCapacity-desc":
  //     filteredData = cabin?.sort((a, b) => b.capacity - a.capacity);
  //     break;
  //   default:
  //     filteredData = cabin;
  //     break;
    ...
    <Menus>
      <Table columns={"0.6fr 1.8fr 2.2fr 1fr 1fr 1fr"}>
        ...
        <Table.Body
          data={sortedData}
          // data={cabin}
          render={(cab: any) => <CabinRow key={cab.id} cabin={cab} />}
        />
      </Table>
    </Menus>
