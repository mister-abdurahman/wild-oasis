...
const StyledSalesChart: any = styled(DashboardBox)`
  grid-column: 1 / -1;
...
function SalesChart({ bookings, numDays }: { bookings: any; numDays: number }) {
  ...
  // returns dates from filter days selected (7,30...) till date.
  ...
  const data = allDates.map((date: Date) => {
    return {
      ...
      totalSales: bookings
        .filter((booking: { created_at: string }) =>
          isSameDay(date, new Date(booking.created_at))
        )
        .reduce(
          (acc: any, cur: { totalPrice: number }) => acc + cur.totalPrice,
          0
        ),
      extrasSales: bookings
        ...
        .reduce(
          (acc: any, cur: { extrasPrice: number }) => acc + cur.extrasPrice,
          0
        ),
    };
  });
  ...
  return (
    <StyledSalesChart isDarkMode={isDarkMode}>
      <Heading as="h2">
